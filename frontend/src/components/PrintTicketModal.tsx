...
export const PrintTicketModal: React.FC<PrintTicketModalProps> = ({ os, onClose }) => {
  ...
  const [empresa, setEmpresa] = useState<any>({
    ...
    razao_social: 'Especializada em Notebooks, Celulares, Impressoras, PCs e Consoles',
  ...
  useEffect(() => {
    fetch('/api/empresa/public')
      .then(res => res.json())
      .then(data => { if (data && data.nome_empresa) setEmpresa(data); })
      .catch(() => {});
  }, []);
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-slate-900 bg-white m-4 rounded-xl print:m-0 print:p-0 print:shadow-none print:text-black">
          ...
          <div className="border-2 border-slate-800 p-4 rounded-xl space-y-3 bg-white">
            ...
            <div className="flex justify-between items-start border-b-2 border-slate-800 pb-3">
              <div>
                <h1 className="font-black text-xl tracking-tight text-slate-950 uppercase">
                  {empresa.nome_empresa}
                </h1>
                <p className="text-xs font-bold text-slate-700">
                  {empresa.razao_social || 'Especializada em Notebooks, Celulares, Impressoras, PCs e Consoles'}
                </p>
                <p className="text-xs text-slate-600">
                  WhatsApp / Contato: <strong>{empresa.whatsapp || empresa.telefone}</strong>
                </p>
              </div>
            ...
            <p className="text-[8.5px] text-slate-600 leading-tight border-t border-slate-200 pt-1.5 text-justify">
              <strong>* TERMO DE GARANTIA E CONDIÇÕES:</strong> {empresa.termo_garantia || 'Garantia legal de 90 dias conforme Art. 26 do Código de Defesa do Consumidor, cobrindo exclusivamente o serviço executado e peças substituídas.'}
            </p>
          </div>

...
export const PrintVendaModal: React.FC<PrintVendaModalProps> = ({ venda, onClose }) => {
  ...
  const msgWhatsApp = encodeURIComponent(
    `Olá, *${venda.cliente_nome || 'Cliente'}*! Aqui está o comprovante da sua compra na *${empresa.nome_empresa}*:\n\n` +
    ...
    `Obrigado pela preferência! Qualquer dúvida, estamos à disposição no WhatsApp ${empresa.whatsapp || empresa.telefone}.`
  );
        ...
        <div className="p-4 overflow-y-auto flex-1 flex justify-center bg-slate-950/50">
          ...
          >
            ...
            <div className="text-center pb-3 border-b-2 border-dashed border-slate-400 space-y-1">
              <h2 className="text-sm font-black tracking-tight text-slate-900 uppercase">{empresa.nome_empresa}</h2>
              <p className="text-[10px] text-slate-600">Assistência Técnica & Venda de Peças</p>
              <p className="text-[10px] text-slate-600">WhatsApp: {empresa.whatsapp || empresa.telefone}</p>
              <div className="pt-1">

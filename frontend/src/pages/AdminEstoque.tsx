...
export const AdminEstoque: React.FC = () => {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      a.href = url;
      a.download = `Estoque_AssistPro_${new Date().toISOString().slice(0, 10)}.xlsx`;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {metricas && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div 
            onClick={() => { setCondicaoFiltro('TODAS'); setApenasUrgente(false); }}
            className={`glass-card rounded-3xl p-5 border transition-all cursor-pointer flex flex-col justify-between hover:scale-[1.02] ${
              !apenasUrgente && condicaoFiltro === 'TODAS'
                ? 'border-brand-500/50 bg-brand-950/20 shadow-md'
          ...
          {/* CARD DE REPOSIÇÃO URGENTE (CLICÁVEL) */}
          <div 
            onClick={() => setApenasUrgente(!apenasUrgente)}
            className={`glass-card rounded-3xl p-5 border transition-all cursor-pointer flex flex-col justify-between group hover:scale-[1.02] ${
              apenasUrgente
                ? 'bg-rose-950/70 border-rose-500 shadow-glow-rose ring-2 ring-rose-500 scale-[1.02]'
                : 'border-white/5 hover:border-rose-500/50 hover:bg-rose-950/20'
            }`}
            title="Clique para filtrar apenas produtos em reposição urgente ou zerados"
          >
            <div className="flex items-center justify-between mb-2">
              <span className={`text-xs font-bold ${apenasUrgente ? 'text-rose-200' : 'text-slate-400 group-hover:text-rose-300'}`}>
                {apenasUrgente ? '🚨 Filtro Ativo' : 'Reposição Urgente'}
              </span>
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 ${
                metricas.itensBaixoEstoque > 0 ? 'bg-rose-500/20 text-rose-400 animate-pulse' : 'bg-slate-800 text-slate-400'
              }`}>
            ...
            <div>
              ...
              <p className="text-[11px] font-semibold text-rose-300 mt-1">
                {apenasUrgente ? '👉 Clique para ver todos os itens' : '👉 Clique para ver os itens críticos'}
              </p>
            </div>
          </div>
      ...
      {/* Banner de Filtro de Urgência Ativo */}
      {apenasUrgente && (
        <div className="p-3.5 bg-gradient-to-r from-rose-950 via-rose-900/80 to-rose-950 border border-rose-500 rounded-2xl text-rose-200 text-xs flex flex-wrap items-center justify-between gap-2 shadow-xl animate-slide-up">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-rose-500/20 text-rose-300 border border-rose-500/30 animate-pulse">
              <AlertTriangle className="w-4 h-4" />
            </div>
            <div>
              <span className="font-bold text-white text-sm block">Exibindo apenas produtos em Reposição Urgente ou Zerados</span>
              <span className="text-[11px] text-rose-300">
                Itens com saldo igual ou abaixo do estoque mínimo configurado.
              </span>
            </div>
          </div>
          <button
            type="button"
            onClick={() => setApenasUrgente(false)}
            className="px-3.5 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs shadow-md transition-all cursor-pointer"
          >
      ...
      <div className="glass-card rounded-3xl p-4 border-white/5 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {/* Abas Novos / Usados / Urgentes */}
          <div className="flex flex-wrap items-center gap-1 p-1 rounded-xl bg-slate-950 border border-white/10 text-xs">
            <button
              type="button"
              onClick={() => { setCondicaoFiltro('TODAS'); setApenasUrgente(false); }}
              className={`px-3 py-1.5 rounded-lg font-bold transition-all cursor-pointer ${
                !apenasUrgente && condicaoFiltro === 'TODAS' ? 'bg-brand-500 text-white shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              Todos ({itens.length})
            </button>
            <button
              type="button"
              onClick={() => { setCondicaoFiltro('NOVO'); setApenasUrgente(false); }}
              className={`px-3 py-1.5 rounded-lg font-bold transition-all cursor-pointer ${
                !apenasUrgente && condicaoFiltro === 'NOVO' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              ✨ Novos
            </button>
            <button
              type="button"
              onClick={() => { setCondicaoFiltro('USADO'); setApenasUrgente(false); }}
              className={`px-3 py-1.5 rounded-lg font-bold transition-all cursor-pointer ${
                !apenasUrgente && condicaoFiltro === 'USADO' ? 'bg-amber-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              🏷️ Usados / Seminovos
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-all cursor-pointer flex items-center gap-1 ${
                apenasUrgente ? 'bg-rose-600 text-white shadow-sm' : 'text-rose-400 hover:text-rose-300'
              }`}
            >
              <AlertTriangle className="w-3.5 h-3.5" />
              <span>Críticos ({metricas?.itensBaixoEstoque || 0})</span>
            </button>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-white/5 text-slate-200">
              {itens
                .filter(item => {
                  if (apenasUrgente && item.quantidade > item.estoque_minimo) return false;
                  return true;
                })
                .map((item) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            <div className="text-[10px] text-slate-400 flex flex-wrap items-center gap-2 mt-0.5">
                              ...
                              {isBaixoEstoque && (
                                <span className={`inline-flex items-center gap-1 px-1.5 py-0.2 rounded text-[9px] font-black uppercase ${
                                  isZerado ? 'bg-rose-600 text-white' : 'bg-rose-500/20 text-rose-300 border border-rose-500/30'
                                }`}>
                                  <AlertTriangle className="w-2.5 h-2.5" />
                                  {isZerado ? 'ESGOTADO' : `REPOSIÇÃO (MÍN: ${item.estoque_minimo})`}
                                </span>
                              )}
                            </div>
                          </div>
                      ...
                      <td className="py-3 px-3 text-center">
                        <div className={`inline-flex items-center gap-1.5 bg-slate-950 px-2 py-1 rounded-xl border ${
                          isBaixoEstoque ? 'border-rose-500/50 shadow-sm' : 'border-white/10'
                        }`}>
                          <button
                          ...
                          <span className={`font-mono font-black px-1 ${
                            isZerado ? 'text-rose-500 font-black animate-pulse' : isBaixoEstoque ? 'text-rose-400 font-black' : 'text-white'
                          }`}>
                            {item.quantidade}
                          </span>
                          <button
              ...
              {itens.filter(item => (apenasUrgente ? item.quantidade <= item.estoque_minimo : true)).length === 0 && !loading && (
                <tr>
                  <td colSpan={8} className="py-8 text-center text-slate-500 text-xs">
                    {apenasUrgente 
                      ? '🎉 Excelente! Não há nenhum produto com estoque baixo ou zerado no momento.' 

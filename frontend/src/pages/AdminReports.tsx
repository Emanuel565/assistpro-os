...
export const AdminReports: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/api/empresa/public')
      .then(res => res.json())
      .then(d => { if (d && d.nome_empresa) setEmpresa(d); })
      .catch(() => {});
  }, []);
    ...
    <div className="space-y-6 animate-fade-in print:p-0 print:space-y-4">
      ...
      <div className="hidden print:block border-b-2 border-slate-900 pb-3 mb-4 text-slate-900">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-xl font-black uppercase tracking-wider">{empresa.nome_empresa}</h1>
            <p className="text-xs text-slate-600">Relatório Administrativo & Fechamento Financeiro • Contato: {empresa.whatsapp || empresa.telefone}</p>
          </div>
        ...
        <div className="space-y-6 print:space-y-4">
          ...
          <div className="hidden print:block pt-8 text-xs text-slate-700 border-t border-slate-300 mt-6">
            <div className="flex justify-between items-end">
              <div>
                <p className="font-bold">AssistPro OS - Gestão para Assistência Técnica</p>

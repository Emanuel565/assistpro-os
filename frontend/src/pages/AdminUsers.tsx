...
import { AdminEmpresaConfig } from '../components/AdminEmpresaConfig';
import { 
  Users, 
  UserPlus, 
  Wrench, 
...
export const AdminUsers: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'EQUIPE' | 'EMPRESA'>('EQUIPE');
  ...
  const handleDownloadBackup = async () => {
    try {
      ...
      a.href = url;
      a.download = `assistpro_backup_${new Date().toISOString().slice(0, 10)}.db`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-black/10 dark:border-white/10">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-white flex items-center gap-2.5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="px-3.5 py-2.5 rounded-full bg-white dark:bg-zinc-900 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-200 border border-black/10 dark:border-white/10 hover:border-emerald-500/40 font-semibold text-xs flex items-center gap-1.5 transition-all cursor-pointer shadow-sm"
          ...
          <button
            ...
            className="apple-button-primary px-5 py-2.5 rounded-full text-white font-semibold text-xs flex items-center gap-2 shadow-lg hover:shadow-blue-500/25 transition-all cursor-pointer shrink-0"
          >
      ...
      {/* Seletor de Aba Apple Pill */}
      <div className="flex items-center gap-1.5 bg-white dark:bg-[#121214] p-1 rounded-full border border-black/10 dark:border-white/[0.08] self-start text-xs shadow-sm">
        <button
          onClick={() => setActiveTab('EQUIPE')}
          className={`px-4 py-2 rounded-full font-semibold transition-all cursor-pointer ${
            activeTab === 'EQUIPE' ? 'bg-zinc-900 text-white dark:bg-white dark:text-black shadow-sm' : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white'
          }`}
        >
          Equipe & Perfis ({users.length})
        </button>
        <button
          onClick={() => setActiveTab('EMPRESA')}
          className={`px-4 py-2 rounded-full font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
            activeTab === 'EMPRESA' ? 'bg-zinc-900 text-white dark:bg-white dark:text-black shadow-sm' : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white'
          }`}
        >
      ...
      {successMsg && (
        <div className="p-3 bg-emerald-100 dark:bg-emerald-950/60 border border-emerald-300 dark:border-emerald-500/40 rounded-xl text-emerald-800 dark:text-emerald-300 text-xs flex items-center gap-2">
        ...
        <>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {users.map((u) => (
                <div key={u.id} className="bg-white dark:bg-[#121214] border border-black/10 dark:border-white/[0.08] hover:border-black/20 dark:hover:border-white/20 rounded-3xl p-5 flex flex-col justify-between relative group transition-all shadow-sm dark:shadow-md">
                  <div>
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex items-center gap-3">
                        <div className="w-11 h-11 rounded-2xl bg-zinc-100 dark:bg-zinc-800 border border-black/10 dark:border-white/20 font-bold text-sm text-zinc-900 dark:text-white flex items-center justify-center shadow-sm shrink-0">
                          {getInitials(u.nome)}
                        </div>
                        <div>
                          <h3 className="font-bold text-zinc-900 dark:text-white text-sm tracking-tight">{u.nome}</h3>
                          <p className="text-xs font-mono text-zinc-500 dark:text-zinc-400">@{u.login}</p>
                        </div>
                      </div>
                      <RoleBadge role={u.cargo} />
                    </div>

                    <div className="space-y-2 text-xs py-2 border-t border-black/5 dark:border-white/[0.08]">
                      {u.telefone && (
                      ...
                      {u.especialidades && u.especialidades.length > 0 && (
                        <div>
                          <span className="text-[10px] text-zinc-500 dark:text-zinc-400 font-semibold uppercase tracking-wider block mb-1">
                            Especialidades de Bancada:
                          </span>
                          <div className="flex flex-wrap gap-1">
                            {u.especialidades.map((esp, i) => (
                              <span key={i} className="text-[10px] px-2.5 py-0.5 rounded-full bg-black/[0.04] dark:bg-white/[0.06] border border-black/5 dark:border-white/10 text-zinc-700 dark:text-zinc-300">
                                {esp}
                              </span>
                            ))}
                          </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-black/5 dark:border-white/[0.08] flex items-center justify-between text-xs">
                    <div className="flex items-center gap-1.5 text-zinc-500 dark:text-zinc-400 text-[11px]">
                      ...
                      <span>OS Ativas: <strong className="text-zinc-900 dark:text-white">{u.os_ativas_count || 0}</strong></span>
                    </div>

                    <div className="flex items-center gap-1.5">
                      <button
                        onClick={() => handleOpenEditModal(u)}
                        className="p-1.5 rounded-full bg-black/[0.04] hover:bg-black/[0.08] dark:bg-white/[0.05] dark:hover:bg-white/[0.1] text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white border border-black/10 dark:border-white/10 transition-colors cursor-pointer"
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-full bg-black/[0.04] hover:bg-amber-100 dark:bg-white/[0.05] dark:hover:bg-amber-950/60 text-zinc-600 hover:text-amber-700 dark:text-zinc-300 dark:hover:text-amber-300 border border-black/10 dark:border-white/10 hover:border-amber-500/40 transition-colors cursor-pointer"
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-full hover:bg-rose-100 dark:hover:bg-rose-950/60 text-zinc-400 hover:text-rose-600 dark:text-zinc-500 dark:hover:text-rose-400 transition-colors cursor-pointer"

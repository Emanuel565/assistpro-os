...
export const downloadDatabaseBackup = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    res.download(dbPath, `assistpro_backup_${timestamp}.db`);
  } catch (error) {
...
// Alternador rápido de modalidade para demonstração e facilidade de uso
export const switchDemo = async (req: Request, res: Response): Promise<void> => {
  try {
    const { userId, cargo, login } = req.body;
    let user = null;

    if (userId) {
      user = await prisma.usuario.findUnique({ where: { id: Number(userId) } });
    } else if (login) {
      user = await prisma.usuario.findUnique({ where: { login: String(login) } });
    } else if (cargo) {
      user = await prisma.usuario.findFirst({ where: { cargo: String(cargo) } });
    }
    ...
    if (!user) {
      res.status(404).json({ error: 'Nenhum perfil de modalidade encontrado.' });
      return;
    }
    ...
    res.status(500).json({ error: 'Erro ao alternar perfil de modalidade.' });
  }
};